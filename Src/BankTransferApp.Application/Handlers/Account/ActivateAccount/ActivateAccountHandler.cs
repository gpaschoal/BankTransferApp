...
public class ActivateAccountHandler(
    ...
    ) : IHandler<ActivateAccountCommand, Result>
{
    public async Task<Result> HandleAsync(
        ActivateAccountCommand request,
        CancellationToken cancellationToken)
    {
        ActivateAccountCommandValidator validator = new();
        ...
        try
        {
            ...
            if (account.OwnerId != userId) return new("Unauthorized", "You are not authorized to activate this account.");
        ...
        {
            logger.LogError(ex, "An error occurred while activating the account with ID {AccountId}", request.AccountId);
